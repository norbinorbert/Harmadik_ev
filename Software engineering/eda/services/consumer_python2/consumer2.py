...
def callback(ch, method, properties, body):
    time.sleep(1)
    print(f" [Python consumer2] Consumer 2 received: {body.decode()}")
...
channel.queue_bind(exchange='choreography_exchange', queue=queue.method.queue)

channel.basic_consume(queue=queue.method.queue, on_message_callback=callback, auto_ack=True)
print(' [Python consumer2] Waiting for messages. To exit press CTRL+C')
